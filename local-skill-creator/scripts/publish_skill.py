...
def main():
    ...
    # 3a. 同步备份到 workspace/skills/
    backup_root = Path.home() / '.openclaw/workspace/skills'
    backup_path = backup_root / skill_name
    if backup_path.exists():
        shutil.rmtree(backup_path)
    shutil.copytree(skill_path, backup_path)
    print(f'💾 Backed up to workspace: {backup_path}')
    ...
    # 4. copy update_skill.sh if exists (keep consistency with local workflow)
    local_update_script = Path.home() / '.openclaw/workspace/local-skill-creator/scripts/update_skill.sh'
    if local_update_script.exists() and (dest / 'update_skill.sh').exists():
        shutil.copy2(local_update_script, dest / 'update_skill.sh')
        print(f'🔧 Copied update_skill.sh to {dest}')
